...
Smoke tests that hit real Spark EDS endpoints (API + page loads) with a session cookie. Tests are defined declaratively in `test-config.js` — covers user, search, reporting, collections, rights requests, saved searches, notifications, content stores, Fadel proxy, and auto-generated Japanese localization variants.
