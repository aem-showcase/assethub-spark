 ...
 *
 * Ported verbatim from cloudflare/src/origin/page-access.js — pure string/logic
 * helpers with no runtime-specific APIs, so nothing to adapt. (The Fastly-specific
 * concern — reading the proxied HTML body without `response.clone()`, which is
 * unavailable on backend responses — lives in the index.js catch-all that calls
 * these, not here.)
 *
