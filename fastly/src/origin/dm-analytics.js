...
export function handleSearchAnalytics(url, request, response, env, ctx) {
  ...
  // CF->Fastly: response.clone() is not available on a backend-fetch response here.
  // Search analytics is log-only in Phase 1a, so skip it rather than fail the search.
  // TODO (2b/1b): read-and-reconstruct (arrayBuffer -> new Response) instead of clone
  // so analytics can read the body without consuming the client response.
  let clonedResponse;
  try {
    clonedResponse = response.clone();
  } catch (err) {
    console.warn('[analytics] response.clone() unavailable — skipping search analytics:', err.message);
    return;
  }
