 ...
 *
 * Ported from cloudflare/src/origin/coa.js. Business logic (envelope building, IMS
 * identity handling, trusted-host gate) is unchanged. Fastly adaptations, isolated:
 *   - both outbound fetches go through the declared-backend router (`fetchBackend`);
 *   - `CacheOverride('pass')` (COA responses are `no-store` anyway).
 * ⚠️ `originCoaImage` fetches whatever host COA returns in `src`. Trusted DM/COA hosts
 *   map to declared backends (`dm_delivery`/`coa`); any other `*.adobe.io` host requires
 *   Dynamic Backends to be enabled on the service (see the plan's T-Pre.6 / Phase 2b).
 *
...
export async function originCoaImage(request, env) {
  ...
  const upstream = await fetchBackend(src, {
