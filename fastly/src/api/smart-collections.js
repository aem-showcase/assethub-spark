 ...
 * withAuthentication (fastly/src/auth.js) before any /api/* route runs, so `request.user`
 * is always populated here; ownership is scoped by request.user.sub (Entra sub, same identity
 * used by audit_events.user_id).
 *
 * Ported from cloudflare/src/api/smart-collections.js. Only change: the shared-types import
 * points at the ported copy under fastly/src/smart-collections/ instead of repo-root scripts/.
 * The `env.SMART_COLLECTIONS` binding is the D1-over-HTTP client (platform/d1-http.js), which
 * exposes the same D1 surface (`.prepare().bind().first()/.all()/.run()`), so the query code
 * is unchanged.
 *
...
import {
  ...
  isValidTitle,
  isValidVisibility,
} from '../smart-collections/smart-collection-types.js';
