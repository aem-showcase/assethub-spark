 ...
 *
 * Ported from cloudflare/src/api/notifications.js. No code changes: it talks to
 * `env.MESSAGES` (the Fastly KV adapter in platform/env.js, which mirrors the
 * Cloudflare KV surface — including `list()` returning `{ keys: [{ name }] }`) and
 * `fetchHelixSheet` (already ported). Phase 1a degrade: KV Store isn't enabled on the
 * PoC account, so the adapter's KV ops no-op/return empty — user notifications persist
 * as nothing and reads return only the EDS **system** notifications, while the API
 * still responds with well-formed JSON. Flips to real persistence the moment a KV
 * Store is linked (no code change).
 */
