...
| `login-page-copied-and-rebranded` | The `login` page and `config` folder are copied, rebranded, and published **under `/<company>`**, so the foldered portal's login is present and on-brand | Step 3–4 / G5 | fixture |
| `copy-verified-path-by-path` | The DA copy is full-tree (incl. `login`/`config`) and verified **path-by-path**, never by a document count that masks a missing subtree | Step 3 / G4 | fixture |
