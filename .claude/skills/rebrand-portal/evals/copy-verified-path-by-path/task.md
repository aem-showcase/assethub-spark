...
tree must come across — including the `login` page document and the
`config` folder — not just the visible page tree.
...
migration this left `/acme/login` and `/acme/config` missing and broke the
...
- It expects a **full recursive** copy of every top-level entry into
  `/acme`, explicitly including the `login` page document and the
  `config` folder, not only the page tree.
...
- Only exit code `0` (copied and verified) advances the step; a
  ...
  as "good enough."
- Plain language to the customer (I1) — no `da-copy-folder.sh`, `login`,
